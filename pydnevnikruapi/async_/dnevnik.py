...
class AsyncDiaryBase:
    ...
    async def get_token(self):
        token_info = await self.session.post(
...
class DiaryAPI:
    ...
    async def get_info(self):
        user_id = await self.api.get("users/me")
        return user_id

    async def get_classmates(self):
        classmates = await self.api.get(f"users/me/classmates")
        return classmates
    ...
    async def get_user_context(self, user_id: int):
        ...
        context = await self.api.get(f"users/{user_id}/context")
        return context

    async def get_user_memberships(self, user_id: int):
        memberships = await self.api.get(f"users/{user_id}/school-memberships")
        return memberships

    async def get_user_education(self, user_id: int):
        education = await self.api.get(f"users/{user_id}/education")
        return education

    async def get_person_memberships(self, person_id: int):
        memberships = await self.api.get(f"persons/{person_id}/school-memberships")
        return memberships

    async def get_schools(self):
        schools = await self.api.get(f"users/me/schools")
        return schools

    async def get_user_schools(self, user_id: int):
        schools = await self.api.get(f"users/{user_id}/schools")
        return schools

    async def get_edu_groups(self):
        edu_groups = await self.api.get(f"users/me/edu-groups")
        return edu_groups

    async def get_user_edu_groups(self, user_id: int):
        edu_groups = await self.api.get(f"users/{user_id}/edu-groups")
        return edu_groups

    async def get_memberships(self):
        schools_memberships = await self.api.get(f"users/me/school-memberships")
        return schools_memberships

    async def get_group_info(self, edu_group_id: int):
        edu_group_info = await self.api.get(f"edu-groups/{edu_group_id}")
        return edu_group_info

    async def get_groups_info(self, edu_groups_list: list):
        edu_groups_info = await self.api.get(
            f"edu-groups", params={"eduGroups": edu_groups_list}
        )
        return edu_groups_info

    async def get_school_groups(self, school_id: int):
        school_edu_groups = await self.api.get(f"schools/{school_id}/edu-groups")
        return school_edu_groups

    async def get_person_groups(self, person_id: int):
        person_edu_groups = await self.api.get(f"persons/{person_id}/edu-groups")
        return person_edu_groups

    async def get_person_groups_all(self, person_id: int):
        person_edu_all = await self.api.get(f"persons/{person_id}/edu-groups/all")
        return person_edu_all

    async def get_person_school_groups(self, person_id: int, school_id: int):
        person_edu_groups_in_school = await self.api.get(
    ...
    async def get_groups_pupils(self, edu_group_id: int):
        edu_groups_list = await self.api.get(f"edu-groups/{edu_group_id}/persons")
        return edu_groups_list

    async def get_groups_parallel(self, group_id: int):
        edu_groups_list = await self.api.get(f"edu-groups/{group_id}/parallel")
        return edu_groups_list

    async def get_group_marks(self, group_id: int):
        edu_groups_marks = await self.api.get(f"edu-groups/{group_id}/final-marks")
        return edu_groups_marks

    async def get_person_group_marks(self, person_id: int, group_id: int):
        #  TODO: check strange response
        person_marks_in_group = await self.api.get(
    ...
    async def get_person_group_marks_final(self, person_id: int, group_id: int):
        person_final_marks_in_group = await self.api.get(
    ...
    async def get_group_subject_final_marks(self, group_id: int, subject_id: int):
        group_subject_marks = await self.api.get(
            f"edu-groups/{group_id}/subjects/{subject_id}/final-marks"
        )
        return group_subject_marks

    async def get_friends(self):
        my_friends = await self.api.get(f"users/me/friends")
        return my_friends

    async def get_user_friends(self, user_id: int):
        user_friends = await self.api.get(f"users/{user_id}/friends")
        return user_friends
    ...
    async def get_lesson_log(self, lesson_id: int):
        lesson_log = await self.api.get(f"lessons/{lesson_id}/log-entries")
        return lesson_log
    ...
    async def post_lesson_log(self, lesson_id: int, lesson_log_entry: str):
        ...
        """
        lesson_log = await self.api.post(
    ...
    async def get_lesson_log_statuses(self):
        lesson_logs_statuses = self.api.get(f"lesson-log-entries/statuses")
        return lesson_logs_statuses

    async def get_lesson_info(self, lesson_id: int):
        lesson_info = await self.api.get(f"lessons/{lesson_id}")
        return lesson_info

    async def get_many_lessons_info(self, lessons_list: list):
        lesson_info = await self.api.post(
    ...
    async def get_marks_histogram(self, work_id: int):
        marks_histogram = await self.api.get(f"works/{work_id}/marks/histogram")
    ...
    async def get_mark_by_id(self, mark_id: int):
        mark_info = await self.api.get(f"marks/{mark_id}")
        return mark_info

    async def get_marks_by_work(self, work_id: int):
        marks = await self.api.get(f"works/{work_id}/marks")
        return marks

    async def get_marks_by_lesson(self, lesson_id: int):
        marks = await self.api.get(f"lessons/{lesson_id}/marks")
        return marks

    async def get_marks_by_lessons(self, lessons_ids: list):
        marks = await self.api.post(f"lessons/marks", data={"lessons": lessons_ids})
        return marks
    ...
    async def get_person_lessons_marks(self, person_id: int, lesson_id: int):
        marks = await self.api.get(f"persons/{person_id}/lessons/{lesson_id}/marks")
        return marks
    ...
    async def get_marks_by_date(self, person_id: int, date=datetime.datetime.now()):
        marks = await self.api.get(f"persons/{person_id}/marks/{date}")
        return marks

    async def get_marks_values(self):
        marks_values = await self.api.get(f"https://api.dnevnik.ru/v2.0/marks/values")
        return marks_values
    ...
    async def get_school_profile(self, school_id: int):
        school_profile = await self.api.get(f"schools/{school_id}")
        return school_profile

    async def get_schools_profiles(self, schools_ids: list):
        school_profiles = await self.api.get(
            f"schools", params={"schools": schools_ids}
        )
        return school_profiles

    async def get_my_schools(self):
        person_schools = await self.api.get(f"schools/person-schools")
        return person_schools

    async def get_all_schools(self):
        all_schools = await self.api.get(f"schools/cities")
        return all_schools

    async def get_school_params(self, school_id: int):
        school_params = await self.api.get(f"schools/{school_id}/parameters")
        return school_params

    async def get_group_subjects(self, group_id: int):
        group_subjects = await self.api.get(f"edu-groups/{group_id}/subjects")
        return group_subjects

    async def get_school_subjects(self, school_id: int):
        school_subjects = await self.api.get(f"schools/{school_id}/subjects")
        return school_subjects

    async def get_task_info(self, task_id: int):
        task_info = await self.api.get(f"tasks/{task_id}")
        return task_info

    async def get_lessons_task(self, lessons_ids: list):
        lessons_task = await self.api.get(f"tasks", params={"lessons": lessons_ids})
        return lessons_task

    async def get_lesson_task(self, lesson_id: int):
        lesson_task = await self.api.get(f"lessons/{lesson_id}/tasks")
        return lesson_task
    ...
    async def get_teacher_students(self, teacher_id: int):
        teacher_students = await self.api.get(f"teacher/{teacher_id}/students")
        return teacher_students

    async def get_school_teachers(self, school_id: int):
        school_teachers = await self.api.get(f"schools/{school_id}/teachers")
        return school_teachers

    async def get_school_timetable(self, school_id: int):
        school_timetable = await self.api.get(f"schools/{school_id}/timetables")
        return school_timetable

    async def get_group_timetable(self, group_id: int):
        group_timetable = await self.api.get(f"edu-groups/{group_id}/timetables")
    ...
    async def get_user_relatives(self, user_id: int):
        user_rel = await self.api.get(f"users/{user_id}/relatives")
        return user_rel

    async def get_relatives(self):
        my_rel = await self.api.get(f"users/me/relatives")
        return my_rel

    async def get_children_relatives(self):
        my_children_relatives = await self.api.get(f"users/me/childrenrelatives")
        return my_children_relatives

    async def get_user_info(self, user_id: int):
        user_info = await self.api.get(f"users/{user_id}")
        return user_info

    async def get_roles(self):
        my_roles = await self.api.get(f"users/me/roles")
        return my_roles

    async def get_user_roles(self, user_id: int):
        user_roles = await self.api.get(f"users/{user_id}/roles")
        return user_roles
    ...
    async def get_lesson_works(self, lesson_id: int):
        lesson_works = await self.api.get(f"works", params={"lesson": lesson_id})
        return lesson_works
    ...
    async def create_lesson_work(self, work: str):
        ...
        """
        lesson_works = await self.api.post(f"works", data={"work": work})
        return lesson_works
